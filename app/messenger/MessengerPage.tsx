...
export default function MessengerPage() {
  ...
  // 2. Обработчики (теперь они не зависят от стейта напрямую, а берут данные из Ref)
  const handleNewMessage = useCallback((msg: any) => {
    const currentActiveChat = activeChatRef.current;

    // Проверяем через Ref, тот ли это чат
    ...
    // Обновляем список чатов (последнее сообщение), даже если чат не активен
    setChats(prev => prev.map(c =>
      c.id === msg.conversationId
        ? { ...c, lastMessage: msg.content, time: 'Just now' }
        : c
    ));
  }, [currentUserId, markAsRead]); // Зависимостей стало меньше
  ...
  // 3. Эффект подписки (теперь он не будет дергаться при каждом чихе)
  useEffect(() => {
    if (!connected || !authChecked) return;

    // Если есть активный чат — заходим в комнату
    if (activeChat?.id) {
    ...
    <div className="bg-background-light dark:bg-background-dark font-sans text-slate-900 dark:text-slate-100 overflow-hidden h-screen flex flex-col">
      ...
      <header className="flex items-center justify-between px-6 py-3 border-b border-white/10 bg-background-light dark:bg-background-dark z-20">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className={`size-2 rounded-full transition-colors ${connected ? 'bg-accent-neon shadow-[0_0_10px_rgba(57,255,20,0.6)]' : 'bg-red-500'
            }`}></div>
